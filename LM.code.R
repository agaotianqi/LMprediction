...
N_folds <- 5      # or 10 if sample size is big
...
for (i in 1:N_folds){
  ...
  data.train <- subset(data.impu,data$group != i)
  
  # data imputation was added in each loop
  ...
  # LASSO selection and model fitting
  ...
  brier_test[i] <- mean((data.test$p_prediction-data.test$Status_event)^2)
